...
handle_call({match,Actor,Service,Command,Domain,Resource,Params}, _From, State) ->
    ...
    io:format("Survey Matched ~s~n",[Resource]),
...
handle_call({intercept,_Domain,_Method,_Path,_Req,_DocRoot}, _From, State) ->
    Reply = {ok,"Survey Interceptor, nothing to see move along now."},
    {reply, Reply, State};
...
process(Actor,Service,Command,Domain,Resource,Params) ->
    lists:flatten([survey:all(Actor,Service,Command,Domain,Resource,Params),
...
matching(Actor,Service,Command,Domain,Resource,Params) when Command =:= create; Command =:= update;Command =:= delete ->
    survey:write(Actor,Service,Command,Domain,Resource,Params);
matching(Actor,Service,Command,Domain,Resource,Params) ->
    survey:read(Actor,Service,Command,Domain,Resource,Params).
