...
-export([match/6,intercept/5,url/0]).
...
intercept(Domain,Method,Path,Req,Actor) ->
    gen_server:call(?MODULE,{intercept,Domain,Method,Path,Req,Actor}).
...
init([]) ->
    ...
    Proxy = case application:get_env(proxy, proxy_url) of
		     {ok,Url} ->
			Url;
		     _ -> 
			 ?PROXY
...
handle_call({match,Actor,Service,Command,Domain,Resource,Params}, _From, State) ->
    ...
    {reply, Reply, State};
handle_call({intercept,Domain,Method,Path,Req,Actor}, _From, Proxy) ->
    Reply = proxy:intercept(Domain,Method,Path,Req,Actor,Proxy),
    {reply, Reply, Proxy};
handle_call(url, _From, Proxy) ->
    {reply, Proxy, Proxy};
...
process(Actor,Service,Command,Domain,Resource,Params) ->
    lists:flatten([proxy:all(Actor,Service,Command,Domain,Resource,Params),
...
matching(Actor,Service,Command,Domain,Resource,Params) when Command =:= create; Command =:= update;Command =:= delete ->
    proxy:write(Actor,Service,Command,Domain,Resource,Params);
matching(Actor,Service,Command,Domain,Resource,Params) ->
    proxy:read(Actor,Service,Command,Domain,Resource,Params).
